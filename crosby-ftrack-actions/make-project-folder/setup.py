...
with open(
    os.path.join(SOURCE_PATH, 'make-project-folder', '_version.py')
) as _version_file:
    VERSION = re.match(
...
STAGING_PATH = os.path.join(
    BUILD_PATH, 'make-project-folder-{0}'.format(VERSION)
)
...
class BuildPlugin(Command):
    ...
    def run(self):
        ...
        result_path = shutil.make_archive(
            os.path.join(BUILD_PATH, 'make-project-folder-{0}'.format(VERSION)),
            'zip',
            STAGING_PATH,
        )
...
# Call main setup.
setup(
    name='crosby-make-project-folder',
    version=VERSION,
    description='Make a folder for the selected project.',
