...
STAGING_PATH = os.path.join(
    BUILD_PATH, 'task-actions-{0}'.format(VERSION)
)
...
class BuildPlugin(Command):
    ...
    def run(self):
        ...
        result_path = shutil.make_archive(
            os.path.join(BUILD_PATH, 'task-actions-{0}'.format(VERSION)),
            'zip',
            STAGING_PATH,
        )
...
# Call main setup.
setup(
    name='crosby-task-actions',
    version=VERSION,
    description='Utility actions concerning tasks.',
